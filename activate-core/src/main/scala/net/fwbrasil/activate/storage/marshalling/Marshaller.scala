...
object Marshaller {
    ...
    def unmarshalling(storageValue: StorageValue, entityValue: EntityValue[_]): EntityValue[_] =
        (storageValue, entityValue) match {
            ...
            case (storageValue: ReferenceStorageValue, entityValue: EntityInstanceEntityValue[_]) =>
                val id = storageValue.value.value.asInstanceOf[Option[BaseEntity#ID]]
            ...
            case (storageValue: ListStorageValue, entityValue: ListEntityValue[_]) =>
                ...
                if (entityValue.emptyValueEntityValue.isInstanceOf[EntityInstanceEntityValue[_]])
                    ReferenceListEntityValue[Any](v.asInstanceOf[Option[List[Option[BaseEntity#ID]]]])(entityValue.valueManifest.asInstanceOf[Manifest[Any]], entityValue.tval.asInstanceOf[Option[Any] => EntityValue[Any]])
                else
            ...
            case (storageValue: ListStorageValue, entityValue: LazyListEntityValue[_]) =>
                val v = storageValue.value.map(list => list.collect {
                    ...
                    new LazyList[BaseEntity](ids.asInstanceOf[List[BaseEntity#ID]])(entityValue.valueManifest.asInstanceOf[Manifest[BaseEntity]])
                }
    ...
    def marshalling(action: StorageAction): ModifyStorageAction =
        action match {
            case action: CreateTable =>
                StorageCreateTable(action.tableName, marshalling(action.idColumn), marshalling(action.columns), action.onlyIfNotExists)
            case action: CreateListTable =>
                StorageCreateListTable(action.ownerTableName, marshalling(action.ownerIdColumn),  action.listTableName, marshalling(action.valueColumn), StorageColumn("POS", new IntStorageValue(None), None), action.onlyIfNotExists)
    ...
    def idMarshalling(entityId: Option[BaseEntity#ID], entityClass: Class[_]): ReferenceStorageValue = {
        val idClass = EntityId.idClassFor(entityClass)
        val tval = EntityValue.tvalFunction[BaseEntity#ID](idClass, classOf[Object])
...
case class StorageColumn(
    name: String,
    storageValue: StorageValue,
    specificTypeOption: Option[String])

sealed trait ModifyStorageAction

case class StorageCreateTable(
    tableName: String,
    idColumn: StorageColumn,
    columns: List[StorageColumn],
    ifNotExists: Boolean)
        extends ModifyStorageAction

case class StorageCreateListTable(
    ownerTableName: String,
    ownerIdColumn: StorageColumn,
    listTableName: String,
    valueColumn: StorageColumn,
    orderColumn: StorageColumn,
    ifNotExists: Boolean)
        extends ModifyStorageAction {
...
case class StorageRemoveListTable(
    listTableName: String,
    ifExists: Boolean)
        extends ModifyStorageAction

case class StorageRenameTable(
    oldName: String,
    newName: String,
    ifExists: Boolean)
        extends ModifyStorageAction

case class StorageRemoveTable(
    name: String,
    ifExists: Boolean,
    cascade: Boolean)
        extends ModifyStorageAction

case class StorageAddColumn(
    tableName: String,
    column: StorageColumn,
    ifNotExists: Boolean)
        extends ModifyStorageAction

case class StorageRenameColumn(
    tableName: String,
    oldName: String,
    column: StorageColumn,
    ifExists: Boolean)
...
case class StorageModifyColumnType(
    tableName: String,
    column: StorageColumn,
    ifExists: Boolean)
        extends ModifyStorageAction

case class StorageRemoveColumn(
    tableName: String,
    name: String,
    ifExists: Boolean)
        extends ModifyStorageAction
...
case class StorageAddReference(
    tableName: String,
    columnName: String,
...
case class StorageRemoveReference(
    tableName: String,
    columnName: String,
    referencedTable: String,
    constraintName: String,
