...
class MultiVMSpecs extends ActivateTest {
    ...
    val coordinatorServerOption = "-Dactivate.coordinator.server=true"
    ...
    "Multiple VMs" should {
        "work with coordinator" in synchronized {
            test(mainVmOptions = List(coordinatorServerOption),
        ...
        "work with offline locking + coordinator" in synchronized {
            test(mainVmOptions = List(optimisticOfflineLockingOption, coordinatorServerOption),
